...
PATTERNS = [
    ...
    {"regex": r"\bstocking\s+densit(?:y|ies)\b", "alternative": "crowding density", "reason": "Industry term for extreme overcrowding in aquaculture tanks/ponds/net pens, where fish are packed at densities causing stress, disease, and cannibalism."},
    {"regex": r"\b(?:fish|shrimp|salmon|tilapia)\s+(?:harvest|market)\s+size\b|\bslaughter\s+weight\b|\bharvest\s+size\b", "alternative": "slaughter size", "reason": "Euphemism for the size/weight at which farmed fish/shrimp are killed ('harvested')."},
