...
PATTERNS = [
    ...
    (r"curiosity\s+killed\s+the\s+cat", "curiosity backfired"),
    (r"like\s+a\s+chicken\s+with\s+(its|their)\s+head\s+cut\s+off", "in a panic"),
    (r"(your|their|his|her)\s+goose\s+is\s+cooked", "you're in trouble"),
    (r"throw(ing|n)?\s+\w+\s+to\s+the\s+wolves", "abandon to criticism"),
    (r"hook,?\s+line,?\s+and\s+sinker", "completely"),
    (r"clip(ping|ped)?\s+(\w+('s)?\s+)?wings", "restrict someone's freedom"),
    (r"(the\s+)?straw\s+that\s+broke\s+the\s+camel'?s\s+back", "the tipping point"),
    ...
    (r"gestation crates?", "pregnancy cage"),
    (r"depopulat(ion|ed|ing)", "mass killing"),
    (r"processing (plants?|facilit(y|ies))", "slaughterhouse"),
    (r"farrowing crates?", "birthing cage"),
    (r"battery cages?", "small wire cage"),
    (r"spent hens?", "discarded hen"),
    (r"(humane(ly)? (slaughter(ed)?|kill(ing|ed)))", "slaughter"),
    (r"broilers?", "chicken raised for meat"),
    ...
    (r"badger(ed|ing|s)?", "pester"),
    (r"ferret(ed|ing)?\s+out", "uncover"),
    ...
    (r"cash\s+cows?", "profit center"),
    (r"sacrificial\s+lambs?", "expendable person"),
    (r"sitting\s+ducks?", "easy target"),
    (r"open\s+season", "free-for-all"),
    (r"put(ting)?\s+(\w+\s+)?out\s+to\s+pasture", "retire"),
    (r"dead\s+ducks?", "lost cause"),
